...
export const LevelUpOverlay: React.FC<LevelUpOverlayProps> = ({
    ...
    const [displayYoc, setDisplayYoc] = useState(oldYoc);
    ...
    useEffect(() => {
        ...
        const createLeafLayer = (radius: number, height: number, yPos: number, color: number) => {
        ...
        const timerId = setTimeout(() => {
            ...
            const updateCounter = (now: number) => {
                ...
                setDisplayYoc(Math.round(currentYoc * 10) / 10);
        ...
        return () => {
            ...
            // Cleanup geometries/materials if needed (optional for simple scene)
        };
    }, [isVisible, oldYoc, newYoc]);

    if (!isVisible) return null;

    return (
        <div className="fixed inset-0 z-[100] bg-emerald-950 overflow-hidden font-sans select-none">
            {/* 3D Canvas Container */}
            ...
            <div className="absolute top-6 right-6 z-[110] flex gap-3">
                {remainingCount > 0 && onSkipAll && (
                    <button
                        onClick={onSkipAll}
                        className="bg-white/10 backdrop-blur-md border border-white/20 text-white/60 px-4 py-2 rounded-full text-xs font-bold hover:bg-white/20 hover:text-white transition-colors"
                    >
                ...
                <button
                    onClick={onClose}
                    className="bg-white/10 backdrop-blur-md border border-white/20 text-white/80 px-4 py-2 rounded-full text-xs font-bold hover:bg-white/20 hover:text-white transition-colors flex items-center gap-1"
                >
                    Skip
            ...
            <div className={`absolute top-20 left-1/2 -translate-x-1/2 w-[85%] transition-all duration-700 transform ${showUI ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0'}`}>
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-2xl overflow-hidden relative">
                    ...
                    <div className="flex flex-col items-center text-center">
                        ...
                        <h1 className="text-2xl font-black text-white mb-1 tracking-tight">🎉 {stockName} 増配発表！</h1>
                        ...
                        <div className="flex items-center gap-6 mb-8">
                            <div className="text-center">
                                ...
                                <p className="text-2xl font-bold text-white/40">{oldYoc.toFixed(1)}%</p>
                            </div>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-5xl font-black text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.5)] transition-all">
                                    {displayYoc.toFixed(1)}<span className="text-xl ml-1">%</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className={`absolute bottom-16 left-0 w-full px-8 transition-all duration-500 delay-500 transform ${isLevelUp ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                <div className="bg-emerald-900/80 backdrop-blur-md border border-emerald-400/30 rounded-2xl p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-white font-bold text-sm">年間配当 <span className="text-emerald-400">+{dividendIncrease.toLocaleString()}円</span></p>
                            <p className="text-emerald-300/70 text-[10px]">{stockName} ({stockCode})</p>
                        </div>
                    </div>
      ...
      `}} />
            ...
            <div className={`absolute bottom-28 left-1/2 -translate-x-1/2 w-full px-8 transition-all duration-500 ${isLevelUp ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
                <button
                    onClick={onClose}
                    className="w-full bg-white text-emerald-950 font-black py-4 rounded-2xl shadow-xl active:scale-95 transition-transform flex items-center justify-center gap-2"
                >
